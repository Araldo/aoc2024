...
group_discount = defaultdict(int)
for x in range(1, X-1):
    for y in range(1, Y-1):
        for direction in DIRECTIONS:
            dx = direction[0]
            dy = direction[1]
            for field_dir in grid[y][x].directions:
                if field_dir in grid[y+dy][x+dx].directions and grid[y][x].group == grid[y+dy][x+dx].group:
                    group_discount[grid[y][x].group] += 1
...
for group, discount in group_discount.items():
    sum -= (discount//2) * group_sizes[group]
print(sum)
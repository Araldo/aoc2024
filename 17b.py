...
while prog != new_prog:
    new_prog = Instructions().run(prog,a,b,c)
    new_detect = 0
    if len(new_prog) < len(prog):
        a += jump
        continue
    for n, m in zip(prog[::-1], new_prog[::-1]):
        if n == m:
            new_detect += 1
        else:
            break
    jump = max(int(math.pow(10, 12 - new_detect)), 1)
    a += jump

for delta in range(1_000):
    a = a - delta
    new_prog = Instructions().run(prog,a,b,c)
    if prog == new_prog:
        print(a)